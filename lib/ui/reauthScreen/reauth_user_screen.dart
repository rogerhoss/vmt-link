...
class _ReAuthUserScreenState extends State<ReAuthUserScreen> {
  ...
  passwordButtonPressed() async {
    if (_passwordController.text.isEmpty) {
      showAlertDialog(context, 'Empty Password'.tr(),
          'Password is required to update email'.tr(), true);
    } else {
      ...
      try {
        ...
        if (result == null) {
          ...
          showAlertDialog(context, 'Couldn\'t verify'.tr(),
              'Please double check the password and try again.'.tr(), true);
        } else {
          if (result.user != null) {
  ...
  facebookButtonPressed() async {
    try {
      ...
      showAlertDialog(
          context, 'Error', 'Couldn\'t verify with facebook.'.tr(), true);
    }
  }
  ...
  appleButtonPressed() async {
    try {
      ...
      if (appleCredential.error != null) {
        showAlertDialog(
            context, 'Error', 'Couldn\'t verify with apple.'.tr(), true);
      }
